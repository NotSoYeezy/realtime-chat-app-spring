...
const closeProfileDropdown = (event) => {
  if (isProfileDropdown.value && event.target.closest('#profile-dropdown-button') === null &&
  ...
  <div class="flex h-screen bg-background-light dark:bg-background-dark font-display overflow-hidden">
    ...
    <main class="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-800 relative">
      <div
        ...
        @click.self="backToChat"
        class="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center transition-opacity duration-300"
      >
        <div class="bg-white dark:bg-slate-900 rounded-lg shadow-2xl max-w-xl w-full max-h-[90%] overflow-y-auto transform transition-all duration-300 scale-100">
          <Settings @back="backToChat" />
        </div>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
              Login as: <span class="font-medium text-primary">{{ currentUser }}</span>
            </p>
          </div>
        </div>
      </header>
    ...
    <aside class="w-64 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700 hidden lg:flex flex-col">
      ...
      <div class="p-4 border-b border-slate-200 dark:border-slate-700">
        <div class="flex items-center justify-start mb-3">
          <h3 class="font-bold text-slate-800 dark:text-white mr-4">Online</h3>
          <span class="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full font-medium">
            {{ Object.keys(onlineUsers).length }}
          </span>

          <div id="profile-dropdown-button" class="relative ml-auto mb-1">

            <button
              @click="toggleProfileDropdown"
              class="h-9 w-9 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center font-bold text-sm text-slate-600 dark:text-slate-300 group-hover:bg-slate-300 transition-colors"
            >
            ...
            <div
              ...
              class="absolute right-0 top-full mt-2 w-48 bg-slate-800 rounded-lg shadow-xl border border-slate-700 z-10"
            >
              <ul class="py-1">
                <li>
                  <a href="#" @click.prevent="openSettings" class="block px-4 py-2 text-sm text-slate-300 hover:bg-slate-700">Settings</a>
                </li>
                <li>
                  <a href="#" @click.prevent="handleLogout" class="block px-4 py-2 text-sm text-red-400 hover:bg-slate-700">Log Out</a>
                </li>
              </ul>
            </div>
          </div>
