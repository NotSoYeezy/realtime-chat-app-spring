...
const activeTab = ref('images') // New: 'images' or 'files'
const mediaImages = ref([])
const mediaFiles = ref([])      // New: stores file URLs
        ...
        <div v-if="!showMedia" class="space-y-6">
          <div class="text-center space-y-3 flex flex-col items-center">
            ...
            <div class="h-20 w-20 relative group/avatar rounded-full overflow-hidden border border-[var(--color-border)]" :class="(isGroupChat && amIAdmin) ? 'cursor-pointer hover:opacity-90' : ''" @click="triggerFileInput">
              ...
              <div v-if="isGroupChat && amIAdmin" class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover/avatar:opacity-100 transition-opacity"><span v-if="!isUploadingImage" class="material-symbols-outlined text-white">photo_camera</span><span v-else class="material-symbols-outlined text-white animate-spin">sync</span></div>
            </div>
            <div v-if="!isEditingName" class="flex items-center justify-center gap-2 group/name">
              ...
              <button v-if="isGroupChat && amIAdmin" @click="isEditingName = true; newGroupName = activeGroup.name" class="opacity-0 group-hover/name:opacity-100 text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-opacity p-1"><span class="material-symbols-outlined text-lg">edit</span></button>
            </div>
            <div v-else class="flex flex-col gap-2 items-center w-full"><input v-model="newGroupName" class="w-full text-center px-3 py-2 bg-[var(--surface-panel-strong)] border border-[var(--color-border)] rounded-lg font-bold" @keyup.enter="handleUpdateName" autoFocus /><div class="flex gap-2 text-sm"><button @click="handleUpdateName" :disabled="isRenaming" class="text-[var(--color-primary)] font-semibold hover:underline">{{ isRenaming ? 'Saving...' : 'Save' }}</button><span class="text-[var(--color-border)]">|</span><button @click="isEditingName = false" class="text-[var(--color-text-secondary)]">Cancel</button></div></div>
          </div>

          <div class="flex justify-center gap-3">
            <button @click="toggleMute" class="flex items-center gap-2 px-4 py-2 rounded-full border transition-colors text-sm font-medium" :class="activeGroup.muted ? 'bg-[var(--color-primary-bg-soft)] text-[var(--color-primary)] border-[var(--color-primary)]' : 'bg-[var(--surface-panel-strong)] text-[var(--color-text-secondary)] border-transparent hover:text-[var(--color-text-primary)]'"><span class="material-symbols-outlined text-[18px]">{{ activeGroup.muted ? 'notifications_off' : 'notifications_active' }}</span>{{ activeGroup.muted ? 'Muted' : 'Mute' }}</button>
          ...
          <div class="space-y-3"><div class="flex items-center justify-between"><label class="text-xs font-bold uppercase text-[var(--color-text-secondary)] tracking-wider">Members ({{ activeGroup.members?.length || 0 }})</label><button v-if="isGroupChat && amIAdmin" @click="$emit('openAddMember')" class="text-xs font-semibold text-[var(--color-primary)] hover:underline flex items-center gap-1"><span class="material-symbols-outlined text-[16px]">add</span> Add People</button></div><div class="bg-[var(--surface-panel-strong)]/50 rounded-xl border border-[var(--color-border)] divide-y divide-[var(--color-border)] overflow-hidden"><div v-for="member in activeGroup.members" :key="member.id" class="flex items-center gap-3 p-3 hover:bg-[var(--surface-panel-strong)] transition-colors group"><div class="h-9 w-9 shrink-0 rounded-full bg-[var(--surface-panel)] flex items-center justify-center text-sm font-bold border border-[var(--color-border)] relative">{{ member.name?.charAt(0) }}<div v-if="isUserAdmin(member)" class="absolute -top-1 -right-1 text-yellow-500 bg-[var(--surface-panel)] rounded-full"><span class="material-symbols-outlined text-[12px] font-bold">crown</span></div></div><div class="flex-1 min-w-0"><p class="text-sm font-medium text-[var(--color-text-primary)] truncate">{{ member.name }} {{ member.surname }}</p><p class="text-xs text-[var(--color-text-secondary)] truncate">@{{ member.username }}</p></div><div v-if="isGroupChat && amIAdmin && member.username !== currentUser" class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button v-if="!isUserAdmin(member)" @click="handlePromoteAdmin(member)" class="p-1.5 rounded-md hover:text-[var(--color-primary)]"><span class="material-symbols-outlined text-[18px]">security</span></button><button v-else @click="handleDemoteAdmin(member)" class="p-1.5 rounded-md hover:text-orange-600"><span class="material-symbols-outlined text-[18px]">person_remove</span></button><button @click="removeMember(member.id)" class="p-1.5 rounded-md hover:text-red-600"><span class="material-symbols-outlined text-[18px]">remove_circle</span></button></div></div></div></div>
          <div v-if="isGroupChat" class="pt-2"><button @click="leaveGroup" class="w-full flex items-center justify-center gap-2 py-3 rounded-lg text-red-500 hover:bg-red-50 font-medium transition-colors border border-transparent hover:border-red-100"><span class="material-symbols-outlined">logout</span> Leave Group</button></div>
        </div>
        ...
        <div v-else class="space-y-4 animate-in slide-in-from-right-4 duration-200">
          ...
          <div class="flex border-b border-[var(--color-border)] mb-4">
            <button
              ...
              :class="activeTab === 'images' ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'"
            >
              Photos
            ...
            <button
              ...
              class="flex-1 pb-3 text-sm font-medium transition-colors relative"
              :class="activeTab === 'files' ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'"
            >
              Files
          ...
          <div v-else-if="activeTab === 'files'">
            ...
            <div v-else class="space-y-2">
              <a
                v-for="(fileUrl, index) in mediaFiles"
                :key="index"
                :href="getFullUrl(fileUrl)"
                target="_blank"
                download
                class="flex items-center gap-3 p-3 rounded-lg bg-[var(--surface-panel-strong)] hover:bg-[var(--color-bg-input)] transition-colors group"
              >
                ...
                <span class="material-symbols-outlined text-[var(--color-text-secondary)] group-hover:text-[var(--color-primary)]">
                  download
                </span>
              </a>
            </div>
